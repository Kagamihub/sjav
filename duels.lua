--// =========================
-- kagamizero HUB (認証システム + デュエルスクリプト + DROP機能1回実行)
-- =========================
...
local _config = {
    password = "kagamizero", -- パスワードを固定
    scriptUrl = "https://raw.githubusercontent.com/Kagamihub/sjav/refs/heads/main/duels.lua",
    title = "kagamizero AUTH SYSTEM"
}
...
local function CreateAuthUI()
    ...
    -- 既存の認証GUIがあれば削除
    local existingGui = CoreGui:FindFirstChild("KagamizeroAuthGate")
    if existingGui then existingGui:Destroy() end
    
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "KagamizeroAuthGate"
    ...
    LoginBtn.BackgroundColor3 = Color3.fromRGB(100, 50, 150)
    LoginBtn.Text = "Login"
    ...
    LoginBtn.MouseButton1Click:Connect(function()
        if Input.Text == _config.password then
            -- パスワード正解
            Input.Text = "Correct!"
            Input.TextColor3 = Color3.fromRGB(0, 255, 0)
            LoginBtn.Visible = false
            LoadingText.Visible = true
            LoadingText.Text = "Loading Duel Script..."
            
            -- デュエルスクリプトを読み込んで実行
            local success, err = pcall(function()
                local duelScript = game:HttpGet(_config.scriptUrl)
                if duelScript and duelScript ~= "" then
                    -- 認証UIを削除
                    ScreenGui:Destroy()
                    -- デュエルスクリプトを実行
                    loadstring(duelScript)()
                    -- メインHUBを実行
                    LoadingText.Text = "Loading kagamizero HUB..."
                    MainScript()
                else
            ...
            if not success then
                LoadingText.Text = "Load Error: " .. tostring(err)
...
--// kagamizero HUB CONFIG SYSTEM

local HttpService = game:GetService("HttpService")

local FolderName = "kagamizero Hub"
...
playButton.MouseButton1Click:Connect(function()
    ...
    if route then
        ...
        playButton.BackgroundColor3 = kagamizeroColor
...
activateStroke.Color = kagamizeroColor
...
activate.MouseButton1Click:Connect(function()
    ...
    if active then
        activate.Text = "ON"
        activate.BackgroundColor3 = kagamizeroColor
...
strokeTop.Color = kagamizeroColor
...
RunService.RenderStepped:Connect(function()
    ...
    topLabel.Text = "kagamizero HUB | FPS "..fps.." | PING "..ping.." ms"
end)
...
strokeHub.Color = kagamizeroColor
...
function createLockGui()
    ...
    lockGui.Name = "kagamizeroBatTarget"
...
local function ShowSection(sectionName)
    ...
    for _,b in pairs(sectionButtons) do
        if b.Text == sectionName then b.BackgroundColor3 = kagamizeroColor end
    end
end
...
local function CreateToggle(sectionName,text)
    ...
    if text == "Radio Steal Nearest" and sectionName == "Settings" then
        ...
        Instance.new("UIStroke", container).Color = kagamizeroColor
        local label = Instance.new("TextLabel")
        ...
        Instance.new("UIStroke", textbox).Color = kagamizeroColor
    ...
    button.MouseButton1Click:Connect(function()
        enabled = not enabled
        if enabled then
            TweenService:Create(button, TweenInfo.new(0.25), {BackgroundColor3 = kagamizeroColor}):Play()
